...
> Eslatma: DEPLOY_SETUP.md da `bright-api` deyilgan. Agar function nomi `server` bo'lsa shuni ishlating; aks holda `bright-api`.
...
# Dashboard → Settings → API → JWT Secret dan oling
...
### 4.2 Environment variables yangilash (agar yangi qo'shsa kerak)

`frontend/.env.example` da yangi `VITE_*` o'zgaruvchilar bor. Netlify'da ham qo'shish kerak:

1. Site → **Site settings** → **Environment variables**
2. **Add a variable** → quyidagilarni qo'shing (kerak bo'lganda):
...
Environment variables o'zgarsa, Netlify avtomatik re-deploy qilmaydi. Qo'lda:

1. **Deploys** → **Trigger deploy** → **Deploy site**
2. Yoki shunchaki bo'sh commit qilib push qiling:
   ```bash
...
Agar xato chiqsa (eng tipik):
- `Module not found` → `npm install` lokalda yangi paket qo'shilgan, push qiling
- `TypeScript error` → lokalda `npm run build` qilib tuzating

### 4.5 Live URL'ni tekshiring

Netlify URL'i (masalan `https://your-site.netlify.app`) ni oching va:
...
Netlify URL'ni oching → login → dashboard. Tema indigo'ga aylangan bo'lishi kerak (agar `globals.css`'ga `theme-indigo-slate.css` import qilingan bo'lsa).

---

## 6. Troubleshooting (eng tez-tez uchraydigan muammolar)

### 6.1 `git push` rejected — "non-fast-forward"

Remote'da yangi commit'lar bor (boshqa joydan push qilingan).
...
```bash
# Lokal migration ro'yxatini sinxronlash
supabase migration repair --status applied <migration_version>
# Yoki avvalgi versiyalarni dry-run qilib ko'ring
...
Eng tipik sabab — `ANTHROPIC_API_KEY` yoki `JWT_SECRET` sozlanmagan. §3.6 ga qaytib tekshiring.
...
npm install
npm run build       # mahalliy build muvaffaqiyatli bo'lsin
...
Migration ishlamadi. Re-run:

```bash
supabase db push
# yoki SQL Editor'da to'g'ridan-to'g'ri:
# Dashboard → SQL Editor → 20260429_phase0_rls_complete.sql kontentini paste qilib Run
```
...
- [ ] **AI chat smoke test:** Claude javob beradi (kb_found: true bo'lishi yaxshi)
- [ ] **AI feedback smoke test:** `{"saved":true}` qaytadi
- [ ] **Netlify:** Deploy "Published", URL ochiladi
- [ ] **Frontend tema:** Indigo accent ko'rinadi (agar globals.css'ga import qilingan bo'lsa)
...
Hozircha Phase 0 muvaffaqiyatini nishonlang ☕️

---

*FIRST_PUSH.md — bugungi changelar uchun deploy sayohati*
*DEPLOY_SETUP.md (asosiy hujjat) ham foydali bo'lishi mumkin: bot R001/R002 nuance lar uchun*
