...
- [ ] **BLOCKED — `ANTHROPIC_API_KEY`:** Sanitized raw CV inputをin-memory orchestrator seamから準備済みprovider-stage pipelineへ渡し、server composition rootへkey/accounting closureを接続してreal staging live smokeを行う。
