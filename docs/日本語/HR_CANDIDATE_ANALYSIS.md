...
> **Status: PARTIAL IMPLEMENTATION / DESIGN.** Public GitHub/cache、bounded local PDF/DOCX、request/role/plan policy、PostgreSQL quota/finally-release、bounded multipart、atomic usage accounting、minimized prompt、strict output/account-before-validation付きinjectable Haiku/Sonnet provider stages、deterministic merge/scoring/report、orchestrator、frontendはreal/tested。Raw CV in-memory seam、server key/accounting composition、live smoke、full HTTP wiringが残り、canonical endpointは`501 NOT_IMPLEMENTED`を返す。現在状態: [STATUS.md](STATUS.md)。
...
> 現行実装はprovider-independent rubric、bounded weighted overall/grade、completeかつ比較可能なGitHub evidenceだけに基づくconservative UZ/JA/EN flagsを提供する。Injectable Sonnet/Haiku stageはmodel/budget/cache policy、minimized prompt、account-before-validation、strict validationを統合し、deterministic finalizeが再計算したoverall/gradeとlocal flagsを保持する。Raw CV seam、server key/accounting composition、live smokeが残る。
