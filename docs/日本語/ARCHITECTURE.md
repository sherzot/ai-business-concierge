...
*HR Candidateはtarget modular pattern。Bounded adapter/request/quota-lifecycle/multipart/orchestrator/application、private in-memory CV seam、minimized prompt/injectable provider-stage/strict output、server key/accounting composition、deterministic merge boundaryはreal、global deadline/full HTTP wiringはpartial scaffold。*
