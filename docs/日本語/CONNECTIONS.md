...
| Telegram bot | 1 | ⚠️ v15 ACTIVE、webhook secret不在時fail-closed `503` | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET` |
...
> Production 2026-08-21: `telegram-bot` v15 ACTIVE。`TELEGRAM_BOT_TOKEN`は存在し、`TELEGRAM_WEBHOOK_SECRET`は不在。GET `200`、POSTは意図的に`503`。Secret設定と`setWebhook`完了までupdatesを受け付けない。
...
### 5.2 Webhook secretと接続の安全なrollout

`scripts/telegram-webhook-rollout.ts`はrandom secretを生成し、mode `0600`のtemporary env-file経由でSupabaseへ設定、Telegram `setWebhook`を呼び出し、exact URL・health・unauthorized POSTを検証する。Telegram commit前のfailureでは新secretを自動unsetする。既存secretの偶発rotationは拒否する。

Bot tokenをshell historyへ残さず入力し、exact production project refを指定する:
...
Helperはtoken/secretをoutputしない。`TELEGRAM_WEBHOOK_SECRET`の手動指定は不要で、defaultは96文字random hex。
...
Netlifyは各PRのpreviewを自動生成する。Preview/branch/dev contextsは別のstaging Supabase projectのみに接続する。Production project refやpublishable keyを`All` contextへ割り当てない。`validate:deploy-env`が不正なcontext/projectペアをbuild時にブロックする。実production dataをstagingへコピーしない。
