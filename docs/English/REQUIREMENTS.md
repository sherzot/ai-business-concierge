...
| R-016 | HR Candidate Analysis | Partial; bounded adapters, local PDF/DOCX plus sanitized raw-CV in-memory seam, request/role, PostgreSQL quota/finally-release lifecycle, multipart, atomic/idempotent usage-cost, minimized prompts, injectable Haiku/Sonnet stages, strict output/account-before-validation, deterministic merge/scorer/report, provider-stage orchestrator/schema, and frontend boundary are ready (backend HR 88/88; frontend 12/12). Server composition/live smoke, active route, and full flow remain | HR |
