...
| R-016 | HR Candidate Analysis | Partial; bounded adapters, request/role, PostgreSQL quota/finally-release lifecycle, multipart, atomic/idempotent usage-cost, minimized injection-resistant prompt and strict output/account-before-validation contracts, deterministic scorer, three-locale evidence report, orchestrator/schema, and frontend boundary are ready (backend HR 76/76; frontend 12/12). Real Haiku/Sonnet invocation, active route, and full flow remain | HR |
