...
| Unit tests | Frontend 28/28 files, 127/127 tests, including HR Candidate 12/12; AI polish/router/usage Deno 18/18; HR GitHub 10 + CV 8 + boundary 5 + quota/lifecycle 12 + multipart 6 + accounting 4 + provider contract 8 + prompt contract 6 + scorer 4 + report 6 + orchestrator 6 + schema 1 = 76/76; targeted backend with Telegram 80/80 |
...
| Remote GitHub Actions | Final code commit `d07577f` main run `32552683005` succeeded in 1m11s: Deno 80/80 plus backend quality, frontend 28/28 files and 127/127 tests, deploy-env 14/14, audit 0 high/critical, a 3,701-module build, and 10-file security green |
...
| HR Candidate Analysis | Partial / provider blocked | Bounded adapters, request/role, PostgreSQL quota/finally-release, multipart, atomic usage/cost persistence, minimized injection-resistant prompts and strict output/account-before-validation, deterministic scoring, three-locale evidence reporting, orchestrator, and frontend boundary are tested; real Haiku/Sonnet invocation, active route, and full flow remain; production `501` |
