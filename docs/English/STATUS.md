...
| Unit tests | Frontend 28/28 files, 127/127 tests, including HR Candidate 12/12; AI polish/router/usage Deno 18/18; HR GitHub 10 + CV 8 + boundary 5 + quota/lifecycle 12 + multipart 6 + accounting 4 + provider contract 8 + prompt contract 6 + provider stages 8 + scorer 4 + report 6 + orchestrator 6 + schema 1 = 84/84; targeted backend with Telegram 88/88 |
...
| Remote GitHub Actions | Final code commit `deadcdc` main run `32553032864` succeeded in 1m18s: Deno 88/88 plus backend quality, frontend 28/28 files and 127/127 tests, deploy-env 14/14, audit 0 high/critical, a 3,701-module build, and 10-file security green |
...
| HR Candidate Analysis | Partial / provider blocked | Bounded adapters, request/role, PostgreSQL quota/finally-release, multipart, atomic usage/cost persistence, minimized prompts, injectable Haiku/Sonnet stage pipeline, strict output/account-before-validation, deterministic merge/scoring/report, orchestrator, and frontend boundary are tested; the raw CV in-memory seam, server composition/live smoke, and active route remain; production `501` |
