...
- `36b9553` `main`ga push qilindi; GitHub CI `32546561166` 1m12sda Deno 51/51, backend quality, frontend 127/127, deploy-env 14/14, audit 0 high/critical, 3,701-module build va 10-file security bilan green. Frontend runtime o'zgarmagani uchun Netlify ataylab skip qilindi.
...
- `f77dd9a` `main`ga push qilindi. GitHub CI `32545770532` 57 soniyada green: frontend 28/28 va 127/127, deploy-env 14/14, audit 0 high/critical, Deno quality, 3,701-module build va 10-file security PASS. Netlify production deploy `6a89065505b5600008dd0385` / build `6a89065505b5600008dd0383` 29 soniyada `ready`, plugin success va 87,145 faylda secret match 0. `/` va `/dashboard/hr/candidates` `200`, CSP mavjud; `index-DipHAHEa.js` bundle production refini 1 marta, staging refini 0 marta va yangi Candidate copy'ni 1 marta saqlaydi.
...
- `private.hr_candidate_quota_state` va `private.hr_candidate_quota_leases` tenant-scoped jadvallari hamda service-role-only reserve/release RPC'lari qo'shildi. Bitta tenant state row locki parallel requestlarni serialize qiladi; minute/day counter accepted requestni sanaydi, concurrency 45 soniyalik expiring lease bilan boshqariladi, stale lease acquire paytida tozalanadi. `anon`/`authenticated` EXECUTE va `service_role` direct-table access yopiq; user FK covering index bilan advisor qarzi yopildi.
- Multipart adapter Content-Type/boundary, duplicate/unknown field, compressed body, MIME/file/text/locale/depth kontraktlarini tekshiradi. `Content-Length` faqat early reject uchun ishlatiladi; chunked stream ham 5 MiB CV + 64 KiB multipart overhead chegarasida o'qish vaqtida to'xtatiladi. Disabled canonical route bounded drain ishlatadi va valid authorized request uchun `501 NOT_IMPLEMENTED`ni saqlaydi; provider/production Edge deploy qilinmadi.
- Deno targeted backend 47/47 PASS; format 17 fayl, check 12 fayl va lint PASS. Monolit check avvalgi 21 ta logging/Hono/risk type xatosini qaytardi, yangi HR satrlarida xato yo'q. Node 22.23.2 muhitida frontend 26/26 fayl va 117/117 test, type-check, deploy-env 14/14, production audit 0 high/critical, 3,701-module build va 10-file security gate PASS.
- Stagingga migration `20260822010759` va FK index follow-up `20260822011030` qo'llandi; migration soni 39 ga yetdi. Remote pgTAP 22-case tranzaksion runner `ok 22`gacha yetib rollback bilan tugadi; read-back 2/2 private table RLS+FORCE, service reserve/release `true`, anon/auth reserve/release `false`, service direct state SELECT `false`. Advisor yangi unindexed-FK topilmasi fixdan keyin yo'q; faqat yangi, hali workload ko'rmagan ikki index uchun kutilgan `unused_index` INFO qoldi. Production DB/Edge o'zgarmadi. Lokal Docker socket javobsiz bo'lgani uchun fresh local 39-migration replay bu sessiyada BLOCKED; staging PostgreSQL 17.6 dry-run/pgTAP bilan verifikatsiya qilindi.
- `398e46e` `main`ga push qilindi; GitHub CI `32543760806` 1m15sda green: Deno 47/47, format/check/lint, frontend 26/26 fayl va 117/117 test, deploy-env 14/14, audit 0 high/critical, 3,701-module build va 10-file security gate PASS. Runtime frontend o'zgarmagani uchun Netlify ataylab skip qilindi.

Qolgan ish: HR provider usage/cost log va frontend upload/result oqimini secretsiz yakunlash; key kelgach semantic CV/scoring/reportni real provider bilan smoke-test qilish, quota lease'ni route `finally`da release qilish va shundan keyin `501`ni olib tashlash.

Fayllar: `.github/workflows/ci.yml`, `supabase/functions/server/index.ts`, `supabase/functions/server/routes/hr-candidate.ts`, `supabase/functions/server/services/hr-candidate/{request-boundary,request-boundary.test,quota,quota.test,http-adapter,http-adapter.test}.ts`, `supabase/migrations/20260822010759_hr_candidate_quota_reservations.sql`, `supabase/migrations/20260822011030_hr_candidate_quota_user_index.sql`, `supabase/tests/database/hr_candidate_quota.test.sql`, HR frontend README va 4-tilli `DEVLOG/STATUS/PLAN/REQUIREMENTS/ARCHITECTURE/HR_CANDIDATE_ANALYSIS`.
...
- `2656e6a` `main`ga push qilindi; GitHub CI `32491296828` 1m9sda green: Deno 34/34, frontend 26/26 fayl va 117/117 test, deploy-env 14/14, audit 0 high/critical, 3,701-modulli build va 10-faylli security gate PASS. Runtime endpointni yoqmaydigan slice bo'lgani uchun Netlify ataylab skip qilindi.
