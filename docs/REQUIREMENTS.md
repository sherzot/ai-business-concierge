...
| R-016 | HR Candidate Analysis | Partial | GitHub/cache, bounded PDF/DOCX, request/role, PostgreSQL quota/finally-release, bounded multipart, atomic/idempotent usage-cost, minimized prompt, injectable Haiku/Sonnet stage pipeline va strict output/account-before-validation, deterministic merge/scorer/report, orchestrator/schema va frontend boundary tayyor (backend HR 84/84; frontend 12/12); raw CV in-memory seam, server key/accounting composition, live smoke, active route va full-flow qolgan |
