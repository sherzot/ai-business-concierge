...
| R-016 | HR Candidate Analysis | Partial | GitHub/cache, bounded PDF/DOCX va sanitized raw CV in-memory seam, request/role, PostgreSQL quota/finally-release, bounded multipart, atomic/idempotent usage-cost, minimized prompt, injectable Haiku/Sonnet stages va strict output/account-before-validation, server key/accounting composition, deterministic merge/scorer/report, provider-stage orchestrator/schema va frontend boundary tayyor (backend HR 91/91; frontend 12/12); application execution, live smoke, active route va full-flow qolgan |
