...
| R-016 | HR Candidate Analysis | Partial | GitHub/cache, bounded PDF/DOCX va sanitized raw CV in-memory seam, request/role, PostgreSQL quota/finally-release, bounded multipart, atomic/idempotent usage-cost, minimized prompt, injectable Haiku/Sonnet stage pipeline va strict output/account-before-validation, deterministic merge/scorer/report, provider-stage orchestrator/schema va frontend boundary tayyor (backend HR 88/88; frontend 12/12); server key/accounting composition, live smoke, active route va full-flow qolgan |
