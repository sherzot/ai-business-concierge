...
| R-016 | HR Candidate Analysis | Partial | GitHub/cache, bounded PDF/DOCX, request/role, PostgreSQL quota/finally-release lifecycle, bounded multipart, atomic/idempotent usage-cost, minimized injection-resistant prompt va strict output/account-before-validation contractlari, deterministik scorer, 3-tilli evidence report, orchestrator/schema va frontend boundary tayyor (backend HR 76/76; frontend 12/12); real Haiku/Sonnet invocation, active route va full-flow qolgan |
