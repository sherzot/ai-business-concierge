...
| R-016 | HR Candidate Analysis | Partial | GitHub/cache, bounded PDF/DOCX va sanitized raw CV in-memory seam, request/role, PostgreSQL quota/finally-release, bounded multipart, atomic/idempotent usage-cost, minimized prompt, injectable Haiku/Sonnet stages va strict output/account-before-validation, server composition, deterministic merge/scorer/report, provider-stage orchestrator/30s-deadline application/schema va frontend boundary tayyor (backend HR 99/99; frontend 12/12); typed provider-unavailable, live smoke, active route va full-flow qolgan |
