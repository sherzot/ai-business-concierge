...
- Binarylar private `generated-documents` bucketida immutable `<tenant>/<user>/documents/<document-id>/document-<storage-version>.<pdf|docx>` yo'lida saqlanadi. `storage_path` CAS parallel export commitlarini, 65 soniyalik `download_expires_at` lease esa faol 60 soniyali signed URL ustidan re-exportni serializatsiya qiladi; lease tugagach superseded object yangi metadata/document commitidan keyin o'chadi. `documents.row_version` parallel edit bilan export publishini CAS qiladi; legacy unversioned yo'llar rollout compatibility uchun o'qiladi. Restrictive Storage policy `anon`/`authenticated` direct accessni yopadi.
