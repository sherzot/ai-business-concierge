...
*HR Candidate — target modular pattern: bounded adapter/request/quota-lifecycle/multipart/orchestrator/30s-deadline application, private in-memory CV seam, minimized prompt/injectable provider-stage/strict output, server key/accounting composition, typed `AI_UNAVAILABLE` и deterministic merge boundaries real; live smoke/full HTTP wiring остаётся partial scaffold.*
