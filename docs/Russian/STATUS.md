...
| Проверка | Состояние |
|---|---|
| Git | PR #3 squash-merged в `main` как `79be466`; Codex-review input-padding hotfix pushed в `main` как `aee6692` |
...
| Visual browser acceptance | Local и production landing copy/locale, no-overflow и no-error checks прошли; computed left padding icon-bearing `pl-8` input равен `32px`; unauthenticated `/admin` безопасно redirects на `/login` |
...
| Admin platform | Partial | Основное управление/monitoring есть; AI-stats `cost/cost_usd` crash fix в production, остаётся authenticated Super Admin smoke-test |
...
1. С production credentials выполнить smoke-test Leader Company Profile и Super Admin dashboard, подтвердив tenant context и AI stats в authenticated flows.
