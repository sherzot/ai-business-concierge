...
| `VITE_SUPABASE_PUBLISHABLE_KEY` | Supabase `sb_publishable_...`, только build scope |

С префиксом `VITE_` допустим только publishable key. `sb_secret_...` и `service_role` запрещены во frontend environment.
