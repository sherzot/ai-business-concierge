...
| R-016 | HR Candidate Analysis | Partial; GitHub/cache, bounded PDF/DOCX, request/role policy, PostgreSQL minute/day/concurrency lease, bounded multipart, orchestrator/schema и frontend upload/state/result boundary готовы (backend HR 43/43; frontend candidate 12/12). Остаются Haiku/Sonnet, usage log, active route wiring и full flow | HR |
