...
| R-016 | HR Candidate Analysis | Partial; bounded adapters, request/role, PostgreSQL quota/finally-release lifecycle, multipart, atomic/idempotent usage-cost, minimized injection-resistant prompt и strict output/account-before-validation contracts, deterministic scorer, three-locale evidence report, orchestrator/schema и frontend boundary готовы (backend HR 76/76; frontend 12/12). Остаются real Haiku/Sonnet invocation, active route и full flow | HR |
