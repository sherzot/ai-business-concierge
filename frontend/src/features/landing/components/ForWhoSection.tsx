      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {t.items.map((item, idx) => (
            <div
              key={item.title}
              className={`rounded-2xl border p-6 flex gap-4 transition-all hover:border-indigo-500/30 ${
            ...
            >
              <div
                className="w-12 h-12 rounded-xl bg-indigo-500/15 flex items-center justify-center flex-shrink-0 text-2xl"
                aria-hidden="true"
              >
                {item.icon}
              </div>
