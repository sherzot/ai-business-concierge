...
import {
  ...
  Calendar, Clock, Layout, List as ListIcon,
  User, Paperclip, Loader2, Pencil, Trash2
} from 'lucide-react';
...
export function TasksPage({ tenant }: { tenant: { id: string } }) {
  const { translate } = useI18n();
  const { session } = useAuthContext();
  const currentUserId = session?.user?.id;

  const { tasks, members, loading, error, reload, create, remove, acknowledge } = useTasks(tenant.id);
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col">
                    ...
                    <td className="px-6 py-4">
                      {task.dueDate && (
                        <div className={clsx("flex items-center gap-1.5 text-sm",
                          isOverdue(task) ? "text-rose-600 font-medium" : "text-slate-600"
                        )}>
                          <Calendar size={14} />
      ...
      <TaskEditModal
        ...
        onSaved={() => { reload(); setEditingTask(null); setCreatingTask(false); }}
      />
      ...
      <AlertDialog open={!!deletingTask} onOpenChange={(o) => !o && setDeletingTask(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{deletingTask?.title}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function TaskCard({ task, currentUserId, onEdit, onDelete, onAcknowledge }: {
  task: Task;
    ...
    <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all group">
        ...
        <div className="flex items-center gap-1">
          {task.dueDate && (
            <div className={clsx("flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded",
              isOverdue(task) ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-500"
            )}>
              <Clock size={10} />
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onEdit(); }}>
                <Pencil size={14} className="mr-2" /> {translate("tasks.editTask")}
              </DropdownMenuItem>
              <DropdownMenuItem variant="destructive" onClick={(e) => { e.stopPropagation(); onDelete(); }}>
                <Trash2 size={14} className="mr-2" /> {translate("tasks.delete")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-50">
        <div className="flex items-center gap-2">
          ...
          {task.acknowledgedAt && <span className="text-[10px] text-emerald-600 font-medium">✓</span>}
        </div>
