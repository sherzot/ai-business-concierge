      ...
      {data && (
        <>
          ...
          <div className={`rounded-xl border px-5 py-4 flex items-center gap-3 ${
            ...
            <div>
              <p className={`font-semibold text-sm ${data.db_latency_ms < 500 ? "text-emerald-800" : "text-amber-800"}`}>
                {data.db_latency_ms < 500 ? "Tizim normal ishlayapti" : "Kechikish sezilmoqda"}
              </p>
              <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
                <Clock size={11} /> DB javob vaqti: <strong>{data.db_latency_ms} ms</strong>
              </p>
            </div>
          </div>
