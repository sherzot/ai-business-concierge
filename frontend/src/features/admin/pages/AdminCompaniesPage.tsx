  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h3 className="text-slate-900 font-semibold text-lg mb-1">Bloklash sababi</h3>
        <p className="text-slate-500 text-sm mb-4">Bu sabab kompaniyaga yuborilmaydi, faqat admin uchun.</p>
        <textarea
          autoFocus
          value={reason}
          ...
          rows={3}
          className="w-full rounded-xl bg-white border border-slate-300 text-slate-900 placeholder-slate-400 px-4 py-3 text-sm resize-none focus:outline-none focus:border-indigo-500 transition-colors mb-4"
        />
        ...
        {(["pending_approval", "active", "suspended", "blocked"] as const).map((s) => (
          ...
          >
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className={`text-xs font-medium ${filter === s ? "" : "text-slate-500"}`}>
                {STATUS_LABELS[s]}
              </span>
            </div>
            <div className={`text-2xl font-bold ${filter === s ? "" : "text-slate-900"}`}>
              {counts[s] ?? 0}
            </div>
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {FILTER_TABS.map((s) => (
          ...
          >
            ...
            <span className="ml-1.5 bg-black/10 rounded-full px-1.5">
